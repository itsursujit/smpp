...
import smpp.actors.SmppClient.{Bind, SendPdu}
import smpp.actors.{SmppClient, SmppClientConfig}
import smpp.extensions.Smpp
import smpp.protocol._
import smpp.userdata.DefaultGsmCharset
  ...
  val clientSup = actorSystem.actorOf(Props(new Actor {
    ...
    override def receive: Receive = {
      ...
      case "bx" =>
        ...
        self ! UserInput
      case "ss" =>
        val dest = Console.in.readLine().trim
        val msg = Console.in.readLine().trim
