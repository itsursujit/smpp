...
import smpp.actors.SmppClient.{Bind, BindFailed, ClientReceive, PeerClosed, PeerTimedOut, SendPdu, UnbindReceived}
import smpp.extensions.Smpp
...
import smpp.protocol.{AtomicIntegerSequenceNumberGenerator, BindReceiver, BindRespLike, BindTransceiver, BindTransmitter, COctetString, CommandStatus, EnquireLink, EnquireLinkResp, EsmeRequest, EsmeResponse, GenericNack, NumericPlanIndicator, Pdu, PduLogger, SmscRequest, SmscResponse, TypeOfNumber, Unbind, UnbindResp}
import smpp.ssl.TlsContext
...
object SmppClient {
  ...
  case class SendMessage(value: String, did: Did, did1: Did)
...
class SmppClient(config: SmppClientConfig, receiver: ClientReceive, pduLogger: PduLogger = PduLogger.default)
  ...
  def connecting: Actor.Receive = {
    case conn@Smpp.OutgoingConnection(remote, local) =>
      ...
      context.parent ! conn
      config.autoBind.foreach {
        self.tell(_, context.parent)
      } // send bind command to yourself if it's configured for autobind
      context.become(bind)
    case Failure(ex) =>
